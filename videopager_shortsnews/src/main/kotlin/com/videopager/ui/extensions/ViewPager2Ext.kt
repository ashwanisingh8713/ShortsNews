...
internal fun ViewPager2.getPageInfo(): Flow<Unit> = callbackFlow {
    val callback = object : ViewPager2.OnPageChangeCallback() {
...
internal fun ViewPager2.getYoutubeUri(): Flow<Unit> = callbackFlow {
    val callback = object : ViewPager2.OnPageChangeCallback() {
        override fun onPageSelected(state: Int) {
            if (scrollState == ViewPager2.SCROLL_STATE_IDLE) return
            trySend(Unit)
...
internal fun ViewPager2.videoCache(): Flow<Unit> = callbackFlow {
    val callback = object : ViewPager2.OnPageChangeCallback() {
        override fun onPageSelected(position: Int) {
            /**
             * Ignore changes that aren't perceived as a change by the user. For example, if
